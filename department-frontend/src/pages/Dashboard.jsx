...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        const res = await fetch(
          `${config.API_BASE_URL}/api/v1/dashboard/my-dashboard`,
          {
            method: "GET",
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(
        `${config.API_BASE_URL}/api/v1/${apiEndpoint}/${itemId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      {loading ? (
        ...
        <div className="text-center bg-white border border-gray-200 rounded-xl p-12 shadow-sm">
          ...
          <p className="text-gray-500 max-w-md mx-auto mb-6">
            Your dashboard is currently empty. Start building your profile by
            adding your first publication, project, or event.
          </p>
          <button
